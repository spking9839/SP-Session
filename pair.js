...
/* ===== SR BOT MINI SESSION ID GENERATOR (BASE64) ===== */
async function generateShortSession(credsPath) {
    try {
        // Read the actual creds.json file
        const credsData = fs.readFileSync(credsPath, 'utf-8');

        // Base64 encode the creds JSON data directly
        const base64Creds = Buffer.from(credsData, 'utf-8').toString('base64');

        // Bot ke format ke according prefix: "SR_BOT_MINI~<data>"
        const sessionId = `SR_BOT_MINI~`;

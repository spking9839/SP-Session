...
/* ===== KNIGHTBOT-COMPATIBLE SESSION ID GENERATOR (GZIP + BASE64) ===== */
async function generateShortSession(credsPath) {
    try {
        // Read the actual creds.json file
        const credsData = fs.readFileSync(credsPath, 'utf-8');

        // Gzip compress the creds JSON first (KnightBot expects this before base64)
        const compressed = zlib.gzipSync(Buffer.from(credsData, 'utf-8'));

        // Then base64 encode the compressed data
        const base64Creds = compressed.toString('base64');

        // KnightBot's required prefix + separator: "KnightBot!<data>"
        const sessionId = `KnightBot!`;

        // Return both session ID prefix and encoded data
        return {
            sessionId: sessionId,
